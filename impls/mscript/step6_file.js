...
const handleDef = (ast) => {
  ...
  return addBinding(symbol, val, globalEnv);
};
...
const wrapInType = (value, env) => {
  switch (true) {
    ...
    case Array.isArray(value):
      return EVAL(new MalList(value), env);
    default:
      return value;
  }
};
    ...
    switch (ast.value[0].value) {
      ...
      case "list": {
        ...
        const evaluatedArgs = args.map((arg) => {
          if (arg instanceof MalSymbol) return arg;
          return EVAL(arg, env);
        });

        return new MalList(evaluatedArgs);
      }
...
const createGlobalEnvironment = () => {
  const env = new Env();
  loadLib(core, env);
  env.setBinding("eval", (ast) => EVAL(ast, env));
